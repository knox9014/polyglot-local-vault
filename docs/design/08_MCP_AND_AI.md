...
AI 제안은 결정론적 제안 엔진과 **같은 승인 UX**를 쓴다 — 사용자 입장에서 흐름이 하나다. 다만 저장 위치는 다르다: 승인을 기다리는 결정론적 제안(R1/R3/R4/R5)은 재생성 가능하므로 `.vault-ai/suggestions/`, AI 제안은 재생성 불가하므로 `.vault/pending.jsonl` 이다. R2/R6 는 승인 없이 자동 반영되므로 이 큐에 들어오지 않는다. (→ `16_SUGGESTION_ENGINE.md` "승인 정책")
